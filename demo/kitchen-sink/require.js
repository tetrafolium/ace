...
(function(global) {
var req, s, head, baseElement, dataMain, src, interactiveScript,
...
function newContext(contextName) {
  var inCheckLoaded, Module, context, handlers, checkLoadedTimeoutId,
      ...
      enabledRegistry = {}, undefEvents = {}, defQueue = [], defined = {},
      urlFetched = {}, bundlesMap = {}, requireCounter = 1,
  ...
  function normalize(name, baseName, applyMap) {
    var pkgMain, mapValue, nameParts, i, j, nameSegment, lastIndex, foundMap,
        foundI, foundStarMap, starI,
        baseParts = baseName && baseName.split('/'),
        normalizedBaseParts = baseParts, map = config.map,
        starMap = map && map['*'];
  ...
  function makeModuleMap(name, parentModuleMap, isNormalized, applyMap) {
    var url, pluginModule, suffix, nameParts,
        ...
        originalName = name, isDefine = true, normalizedName = '';
      ...
      if (prefix) {
        if (pluginModule && pluginModule.normalize) {
          ...
          normalizedName = pluginModule.normalize(
              name,
              function(name) { return normalize(name, parentName, applyMap); });
        } else {
    ...
    'module' : function(mod) {
      if (mod.module) {
        ...
        return (mod.module = {
          ...
          uri : mod.map.url,
          config :
              function() { return getOwn(config.config, mod.map.id) || {}; },
  ...
  function checkLoaded() {
    var err, usingPathFallback, waitInterval = config.waitSeconds * 1000,
                                // It is possible to disable the wait interval
                                // by using waitSeconds of 0.
        expired = waitInterval &&
                  (context.startTime + waitInterval) < new Date().getTime(),
                                noLoads = [], reqCalls = [],
                                stillLoading = false, needCycleCheck = true;
    ...
    if (needCycleCheck) {
      each(reqCalls, function(mod) { breakCycle(mod, {}, {}); });
    }
  ...
  Module.prototype = {
    init : function(depMaps, factory, errback, options) {
      ...
      if (errback) {
        ...
        errback = bind(this, function(err) { this.emit('error', err); });
      }
    ...
    callPlugin : function() {
      ...
      on(pluginMap, 'defined', bind(this, function(plugin) {
           ...
           if (this.map.unnormalized) {
             ...
             normalizedMap =
                 makeModuleMap(map.prefix + '!' + name, this.map.parentMap);
             ...
             if (normalizedMod) {
               ...
               if (this.events.error) {
                 normalizedMod.on(
                     'error',
                     bind(this, function(err) { this.emit('error', err); }));
               }
           ...
           load.fromText = bind(this, function(text, textAlt) {
             ...
             try {
               ...
               return onError(makeError(
                   'fromtexteval', 'fromText eval for ' + id + ' failed: ' + e,
                   e, [ id ]));
             }
      ...
      each(this.depMaps, bind(this, function(depMap, i) {
             ...
             if (typeof depMap === 'string') {
               ...
               depMap = makeModuleMap(
                   depMap, (this.map.isDefine ? this.map : this.map.parentMap),
                   false, !this.skipMap);
    ...
    emit : function(name, evt) {
      each(this.events[name], function(cb) { cb(evt); });
      if (name === 'error') {
  ...
  function intakeDefines() {
    ...
    while (defQueue.length) {
      ...
      if (args[0] === null) {
        return onError(
            makeError('mismatch', 'Mismatched anonymous define() module: ' +
                                      args[args.length - 1]));
      } else {
  ...
  context = {
      ...
      mixin(localRequire, {
        ...
        toUrl : function(moduleNamePlusExt) {
          ...
          return context.nameToUrl(
              normalize(moduleNamePlusExt, relMap && relMap.id, true), ext,
              true);
        },
    ...
    nameToUrl : function(moduleName, ext, skipExt) {
      ...
      // arg (i.e. ?) or ends with .js, then assume the user meant to use an url
      // and not a module id. The slash is important for protocol-less URLs as
      // well as full paths.
      if (req.jsExtRegExp.test(moduleName)) {
        // Just a plain path, not module name lookup, so just return it.
        // Add extension if it is included. This is a bit wonky, only non-.js
        // things pass an extension, this method probably needs to be reworked.
        url = moduleName + (ext || '');
      } else {
     ...
     * @private
     */
    execCb : function(name, callback, args,
                      exports) { return callback.apply(exports, args); },
...
req.load = function(context, moduleName, url) {
  ...
  if (isBrowser) {
    ...
    try {
      ...
      context.onError(
          makeError('importscripts',
                    'importScripts failed for ' + moduleName + ' at ' + url, e,
                    [ moduleName ]));
    }
  }
};
