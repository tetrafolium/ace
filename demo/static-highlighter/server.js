/**
 * Simple node.js server, which generates the synax highlighted version of
 * itself using the Ace modes and themes on the server and serving a static web
 * page.
 */
...
http.createServer(function(req, res) {
      ...
      fs.readFile(path, "utf8", function(err, data) {
        ...
        console.log("Rendered " + (data.length / 1000) + "kb in " +
                    (Date.now() - t) + "ms");
        res.end('<html><body>\n' +
                '<style type="text/css" media="screen">\n' + highlighted.css +
                '</style>\n' + highlighted.html + '</body></html>');
      });
    })
    .listen(port);
