...
var options = {
  ...
  linkFormat : function(linkHtml) {
    ...
    if (o !== null) {
      href =
          href.replace(href, '#nav=api&api=' + o[1]); // + '&section=' + o[2]);
    }
  ...
  exclude : [
    "**/*_test.js", "**/mode/**", "default_commands.js",
    "multi_select_commands.js", "**/test/**", "**/theme/**", "**/worker/**"
  ],
  index : "./index.md"
};
        ...
        files.forEach(function(file) {
            if (file.match(/\.html$/)) {
                ...
                fs.readFile(outFile, "utf8", function(err, data) {
                    var otherPageRegExp = new RegExp('<a
    href="(.+?.html)(#.+?)?"', "g"); var m;

                    if ( (m = data.match(otherPageRegExp)) ) {
                        console.log(m)
                        data = data.replace(otherPageRegExp, '<a
    href="#nav=api&api=' + m[1] + '&section=' + m[2] + '"')
                    }
