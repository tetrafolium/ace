...
$(function() {
  if (typeof ace !== "undefined") {
    ...
    ace.config.loadModule("ace/ext/emmet", function() {
      ace.require("ace/lib/net")
          .loadScript("https://cloud9ide.github.io/emmet-core/emmet.js",
                      function() {
    ...
    ace.config.loadModule("ace/ext/language_tools", function() {
      embedded_editor.setOptions(
          {enableSnippets : true, enableBasicAutocompletion : true});
      editor.setOptions(
          {enableSnippets : true, enableBasicAutocompletion : true});
    });
  } else {
  ...
  function magicClickInterceptor(e) {
    ...
    var state = {};
    state.api =
        $(this).attr("href").substring(6, $(this).attr("href").length - 5);
    $.bbq.pushState(state);
    ...
    $("#apiHolder").load($(this).attr("href") + " #documentation", function() {
      ...
      if (section) {
        $("li#dropdown_" + section.replace(/\./g, '\\.') + " a")
  ...
  $('a.external').click(function(e) { e.preventDefault(); });

  var tabs = $("#tabnav"), tab_a_selector = "a";
  ...
  tabs.find(tab_a_selector).click(function(e) {
    ...
    function scrollIntoPosition(el, overridePosition) {
      if (typeof overridePosition !== "undefined") {
        $("body").stop().animate({scrollTop : overridePosition}, 400);
      } else if ($("body").scrollTop() > 345) {
        $("body").stop().animate({scrollTop : ($(el).offset().top - 10)}, 400);
      }
    }
  ...
  $('#tabnav a[data-toggle="tab"]').on('shown', function(e) {
    $(".tab-content .tab-pane.active .ace_editor")
        .each(function(i, el) { el.env.onResize(); });
  });
      ...
      .on("hashchange",
          function(e) {
            ...
            tabs.each(function() {
              ...
              $(this)
                  .find(tab_a_selector + "[href='#" + idx + "']")
                  .triggerHandler('click');

              // handles dropping in from new link
              var api = $.bbq.getState("api");
              if (api) {
                $(tab_a_selector + "[href='./api/" + api + ".html']")
                    .triggerHandler('click');
              }
            });
          })
...
function highlight() {
  ...
  qsa("code[class]").forEach(function(el) {
    ...
    if (!m)
      return var mode = "ace/mode/" + (m[1] || m[2]);
