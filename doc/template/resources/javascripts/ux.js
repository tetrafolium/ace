$(function() {
  ...
  var pagePath = document.location.pathname.substring(
      ...
      form.onsubmit = function(evt) {
        ...
        if (query) {
          ...
          var url = "https://www.google.com/search?q=" +
                    encodeURIComponent("site:ace.ajax.org" +
                                       " " + query);
          window.open(url);
        }
        return false;
...
function ux() {
  ...
  function clearMenus() {
    $(d).parent('li').each(function() { $(this).removeClass('open') });
  }
  ...
  window.onhashchange = function() { showMethodContent(); }
};
...
function scrollTo(el, data) {
  ...
  if (document.body.scrollTop > top ||
    ...
    $('body').animate({scrollTop : top}, {duration : 200, easing : "swing"});
  }
}