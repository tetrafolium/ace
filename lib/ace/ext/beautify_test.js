...
define(function(require, exports, module) {
  ...
  // Execution ORDER: test.setUpSuite, setUp, testFn, tearDown,
  // test.tearDownSuite
  module.exports = {
    timeout : 10000,

    "test beautify first line empty" : function() {
      var s = new EditSession([ "", "hello world" ], new PHPMode());
    ...
    "test beautify block tag indentation" : function() {
      var s = new EditSession([ "<div>", "<h1>hello</h1>", "world</div>" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify block tag line breaks and indentation" : function() {
      var s = new EditSession([ "<html><body><div></div></body></html>" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify empty block tag" : function() {
      var s = new EditSession([ "\t<div></div>" ], new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify inline tag indentation" : function() {
      var s = new EditSession([ "<div>", "<span>hello world</span>", "</div>" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify singleton tag indentation" : function() {
      var s = new EditSession([ "<div>", "hello<br>", "world", "</div>" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify unknown singleton indentation" : function() {
      var s = new EditSession([ "<div>", "hello<single />", "world", "</div>" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify adding bracket whitespace" : function() {
      var s = new EditSession([ "<?php", "if(true){", "\t$i++;", "}" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify adding keyword whitespace" : function() {
      var s = new EditSession([ "<?php", "if ($foo===true) {", "\t$i++;", "}" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify switch indentation" : function() {
      var s = new EditSession(
          [
            "<?php", "switch ($i) {", "case 1;", "case 2;", "print $foo;",
            "break;", "case 2;", "print $bar;", "}"
          ],
          new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify multiline string" : function() {
      var s = new EditSession([ "<?php", "\tprint 'hello", "\t\tworld'" ],
                              new PHPMode());
      s.setUseSoftTabs(false);
    ...
    "test beautify css" : function() {
      ...
      s.setValue(
          ".x    y:h{    animation: appear 1.5s ease-in-out     ease-in-out;     border:  solid    red;}");
    ...
    "test beautify js object" : function() {
      var s = new EditSession([ '<script>{"a": "1", "b": "2"}</script>' ],
                              new PHPMode());
      s.setUseSoftTabs(false);
