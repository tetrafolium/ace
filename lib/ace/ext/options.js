define(function(require, exports, module) {
  ...
  var modes = modelist.modes.map(function(
      x) { return {caption : x.caption, value : x.mode}; });

  var optionGroups = {
    Main : {
      Mode : {path : "mode", type : "select", items : modes},
      Theme : {path : "theme", type : "select", items : themes},
      "Keybinding" : {
        type : "buttonBar",
      ...
      "Font Size" : {
        ...
        defaultValue : 12,
        defaults :
            [ {caption : "12px", value : 12}, {caption : "24px", value : 24} ]
      },
      "Soft Wrap" : {
        type : "buttonBar",
        path : "wrap",
        items : [
          {caption : "Off", value : "off"}, {caption : "View", value : "free"},
      ...
      "Overscroll" : {
        ...
        path : "scrollPastEnd",
        items : [
          {caption : "None", value : 0}, {caption : "Half", value : 0.5},
    ...
    More : {
      ...
      "Full Line Selection" :
          {type : "checkbox", values : "text|line", path : "selectionStyle"},
  ...
  (function() {
    ...
    this.renderOptionGroup = function(group) {
      return Object.keys(group)
          ...
          .map(function(item) { return this.renderOption(item.label, item); },
               this);
    };
    ...
    this.renderOptionControl = function(key, option) {
      ...
      if (Array.isArray(option)) {
        return option.map(function(
            x) { return self.renderOptionControl(key, x); });
      }
      var control;
      ...
      if (option.values && option.type != "checkbox") {
        ...
        option.items =
            option.values.map(function(v) { return {value : v, name : v}; });
      }

      if (option.type == "buttonBar") {
        control = [
          "div", {role : "group", "aria-labelledby" : option.path + "-label"},
            ...
            return [
              "button", {
                ...
                onclick : function() {
                  ...
                  var nodes =
                      this.parentNode.querySelectorAll("[ace_selected_button]");
        ...
        control = [
          "input", {
            ...
            style : "width:3em",
            oninput :
                function() { self.setOption(option, parseInt(this.value)); }
          }
        ];
        if (option.ariaLabel) {
        ...
        if (option.defaults) {
          control = [
            control, option.defaults.map(function(item) {
              return [
                "button", {
                  onclick : function() {
        ...
        var items =
            Array.isArray(option.items)
                ...
                : Object.keys(option.items).map(function(key) {
                    return [
                      "optgroup", {"label" : key}, buildItems(option.items[key])
                    ];
                  });
        control = [
          "select", {
            id : key,
            value : value,
            onchange : function() { self.setOption(option, this.value); }
          },
          items
        ];
      } else {
