define(function(require, exports, module) {
  ...
  /**
   * Whenever the selection is changed, prevent cursor (lead) to be positioned
   *at position 0 of right-to-left line in order to maintain the RLE marker at
   *this position. When cursor reaches position 0, either advance it to position
   *1 of current line (default) or to last position of previous line (if it
   *comes from position 1 as the result of commands mentioned in
   *'onCommandEmitted' event handler). This serves few purposes:
   * - ensures cursor visual movement as if RLE mark doesn't exist.
   * - prevents character insertion before RLE mark.
   * - prevents RLE mark removal when 'delete' is pressed when cursot stays at
   *position 0.
   * - ensures RLE mark removal on line merge, when 'delete' is pressed and
   *cursor stays at last position of previous line and when 'backspace' is
   *pressed and cursor  stays at first position of current line. This is achieved
   *by hacking range boundaries on 'remove' operation.
   **/
    ...
    if (editor.session.$bidiHandler.isRtlLine(lead.row)) {
      if (lead.column === 0) {
        if (editor.session.$bidiHandler.isMoveLeftOperation && lead.row > 0) {
          editor.getSelection().moveCursorTo(
              lead.row - 1, editor.session.getLine(lead.row - 1).length);
        } else {
  ...
  function onChange(delta, editor) {
    ...
    if (session.$bidiHandler.isRtlLine(delta.start.row) &&
        delta.action === 'insert' && delta.lines.length > 1) {
