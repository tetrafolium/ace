...
define(function(require, exports, module) {
  ...
  CodeMirror.keyName = function(e) {
    ...
    if (key.length == 1)
      key = key.toUpperCase();
    key = event.getModifierString(e).replace(
  ...
  (function() {
    ...
    this.onChange = function(delta) {
      ...
      if (!curOp.changeHandlers)
        curOp.changeHandlers = this._eventRegistry["change"] &&
    ...
    this.setSelections = function(p, primIndex) {
      ...
      for (var i = 0; i < ranges.length; i++) {
        var range = session.$clipRangeToDocument(
            ranges[i]); // todo why ace doesn't do this?
        sel.addRange(range);
      }
    };
    ...
    this.somethingSelected = function(
        p) { return !this.ace.selection.isEmpty(); };
    ...
    this.findPosV = function(start, amount, unit, goalColumn) {
      ...
      if (unit == 'line') {
        var screenPos =
            this.ace.session.documentToScreenPosition(start.line, start.ch);
        if (goalColumn != null)
          screenPos.column = goalColumn;
        screenPos.row += amount;
        // not what codemirror does but vim mode needs only this
        screenPos.row = Math.min(Math.max(0, screenPos.row),
                                 this.ace.session.getScreenLength() - 1);
        var pos = this.ace.session.screenToDocumentPosition(screenPos.row,
                                                            screenPos.column);
        return toCmPos(pos);
      } else {
        debugger;
    ...
    this.scrollTo = function(x, y) {
      ...
      maxHeight -= (renderer.$size.scrollerHeight - renderer.lineHeight) *
    ...
    this.replaceRange = function(text, s, e) {
      if (!e)
        e = s;
      return this.ace.session.replace(new Range(s.line, s.ch, e.line, e.ch),
                                      text);
    };
    ...
    this.getSelection = function() { return this.ace.getSelectedText(); };
    this.getSelections = function() {
      return this.listSelections().map(
          function(x) { return this.getRange(x.anchor, x.head); }, this);
    };
    this.getInputField = function() { return this.ace.textInput.getElement(); };
    this.getWrapperElement = function() { return this.ace.container; };
    var optMap = {
    ...
    this.addOverlay = function(o) {
      if (!this.$searchHighlight || !this.$searchHighlight.session) {
        var highlight =
            new SearchHighlight(null, "ace_highlight-marker", "text");
    ...
    this.getValue = function() { return this.ace.getValue(); };
    this.setValue = function(v) { return this.ace.setValue(v, -1); };
    ...
    this.indexFromPos = function(
        pos) { return this.ace.session.doc.positionToIndex(toAcePos(pos)); };
    this.posFromIndex = function(
        index) { return toCmPos(this.ace.session.doc.indexToPosition(index)); };
    this.focus = function(index) { return this.ace.textInput.focus(); };
    this.blur = function(index) { return this.ace.blur(); };
    this.defaultTextHeight = function(
        index) { return this.ace.renderer.layerConfig.lineHeight; };
    this.scanForBracket = function(pos, dir, _, options) {
      var re = options.bracketRegex.source;
      var tokenRe = /paren|text|operator|tag/;
      if (dir == 1) {
        var m = this.ace.session.$findClosingBracket(re.slice(1, 2),
                                                     toAcePos(pos), tokenRe);
      } else {
        var m = this.ace.session.$findOpeningBracket(
            re.slice(-2, -1), {row : pos.line, column : pos.ch + 1}, tokenRe);
      }
      return m && {pos : toCmPos(m)};
    };
    this.refresh = function() { return this.ace.resize(true); };
    this.getMode = function() { return {name : this.getOption("mode")}; };
  ...
  function toAcePos(cmPos) { return {row : cmPos.line, column : cmPos.ch}; }
  function toCmPos(acePos) { return new Pos(acePos.row, acePos.column); }
  ...
  StringStream.prototype = {
    ...
    match : function(pattern, consume, caseInsensitive) {
      if (typeof pattern == "string") {
        var cased = function(
            str) { return caseInsensitive ? str.toLowerCase() : str; };
  ...
  CodeMirror.defineExtension = function(
      name, fn) { CodeMirror.prototype[name] = fn; };
...
.ace_dialog input {\
  ...
  var defaultKeymap = [
    ...
    // the next two aren't motions but must come before more general motion
    // declarations
    {
      keys : ']p',
      type : 'action',
  ...
  var wordCharTest =
      ...
      bigWordCharTest = [ function(ch) { return /\S/.test(ch); } ];
  ...
  function isEndOfSentenceSymbol(k) { return '.?!'.indexOf(k) != -1; }
  ...
  var vimApi = {
    ...
    getRegisterController :
        function() { return vimGlobalState.registerController; },
    // Testing hook.
    resetVimGlobalState_ : resetVimGlobalState,

    // Testing hook.
    getVimGlobalState_ : function() { return vimGlobalState; },
    ...
    unmap : function(lhs, ctx) { exCommandDispatcher.unmap(lhs, ctx); },
    ...
    noremap : function(lhs, rhs, ctx) {
      ...
      for (var i = actualLength - origLength;
           i < actualLength && ctxsToMap.length; i++) {
        var mapping = defaultKeymap[i];
        // Omit mappings that operate in the wrong context(s) and those of
        // invalid type.
        if (mapping.keys == rhs &&
          ...
          ctxsToMap = ctxsToMap.filter(function(
              el) { return mappedCtxs.indexOf(el) === -1; });
        }
      }
      // TODO: Create non-recursive keyToKey mappings for the unmapped contexts
      // once those exist.
    },
    ...
    // TODO: Expose setOption and getOption as instance methods. Need to decide
    // how to namespace them, or somehow make them work with the existing
    // CodeMirror setOption/getOption API.
    setOption : setOption,
    getOption : getOption,
    ...
    defineEx : function(name, prefix, func) {
      if (!prefix) {
        ...
        throw new Error('(Vim.defineEx) "' + prefix + '" is not a prefix of "' +
    ...
    findKey : function(cm, key, origin) {
      ...
      function handleKeyInsertMode() {
        ...
        while (keys.length > 1 && match.type != 'full') {
          ...
          var thisMatch = commandDispatcher.matchCommand(
              keys, defaultKeymap, vim.inputState, 'insert');
        ...
        if (keysAreChars) {
          ...
          for (var i = 0; i < selections.length; i++) {
            ...
            cm.replaceRange('', offsetCursor(here, 0, -(keys.length - 1)), here,
                            '+input');
          }
      ...
      function handleKeyNonInsertMode() {
        ...
        var match = commandDispatcher.matchCommand(
            keysMatcher[2] || keysMatcher[1], defaultKeymap, vim.inputState,
            context);
        if (match.type == 'none') {
          clearInputState(cm);
          return false;
    ...
    handleEx : function(
        cm, input) { exCommandDispatcher.processCommand(cm, input); },
  ...
  Register.prototype = {
    ...
    pushSearchQuery : function(query) { this.searchQueries.push(query); },
    clear : function() {
      this.keyBuffer = [];
      this.insertModeChanges = [];
      this.searchQueries = [];
   ...
   *
   * The name should be a single character that will be used to reference the
   * register. The register should support setText, pushText, clear, and
   * toString(). See Register for a reference implementation.
   */
  ...
  RegisterController.prototype = {
    ...
    isValidRegister : function(
        name) { return name && inArray(name, validRegisters); },
  ...
  HistoryController.prototype = {
    ...
    nextMatch : function(input, up) {
      ...
      for (var i = this.iterator + dir; up ? i >= 0 : i < historyBuffer.length;
           i += dir) {
  ...
  var commandDispatcher = {
    ...
    processSearch : function(cm, vim, command) {
      ...
      function handleQuery(query, ignoreCase, smartCase) {
        ...
        commandDispatcher.processMotion(cm, vim, {
          ...
          motion : 'findNext',
          motionArgs :
              {forward : true, toJumplist : command.searchArgs.toJumplist}
        });
      }
      ...
      function onPromptKeyUp(e, query, close) {
        ...
        if (keyName == 'Up' || keyName == 'Down') {
          ...
          if (keyName != 'Left' && keyName != 'Right' && keyName != 'Ctrl' &&
              keyName != 'Alt' && keyName != 'Shift')
            vimGlobalState.searchHistoryController.reset();
        }
        var parsedQuery;
        try {
          parsedQuery = updateSearchQuery(cm, query, true /** ignoreCase */,
                                          true /** smartCase */);
        } catch (e) {
      ...
      case 'wordUnderCursor':
        var word = expandWordUnderCursor(
            cm, false /** inclusive */, true /** forward */,
            false /** bigWord */, true /** noSymbol */);
        var isKeyword = true;
        if (!word) {
          word = expandWordUnderCursor(
              cm, false /** inclusive */, true /** forward */,
              false /** bigWord */, false /** noSymbol */);
          isKeyword = false;
        }
        if (!word) {
          return;
        ...
        var query =
            cm.getLine(word.start.line).substring(word.start.ch, word.end.ch);
    ...
    evalInput : function(cm, vim) {
      ...
      var sel = vim.sel;
      // TODO: Make sure cm and vim selections are identical outside visual
      // mode.
      var origHead =
          copyCursor(vim.visualMode ? clipCursorToContent(cm, sel.head)
                                    : cm.getCursor('head'));
      var origAnchor =
          copyCursor(vim.visualMode ? clipCursorToContent(cm, sel.anchor)
                                    : cm.getCursor('anchor'));
      ...
      if (operator) {
        if (operatorArgs.lastSel) {
          ...
          if (lastSel.visualLine) {
            ...
            // Normal Visual mode with several lines: The same number of lines,
            // in the last line the same number of characters as in the last
            // line the last time.
            newHead = Pos(oldAnchor.line + lineOffset, oldAnchor.ch);
          }
          vim.visualMode = true;
          ...
          sel = vim.sel = {anchor : newAnchor, head : newHead};
          updateCmSelection(cm);
        ...
        if (vim.visualMode) {
          ...
          cmSel = makeCmSelection(cm, {anchor : curStart, head : curEnd}, mode);
          if (linewise) {
        ...
        var operatorMoveTo = operators[operator](cm, operatorArgs, cmSel.ranges,
                                                 oldAnchor, newHead);
        if (vim.visualMode) {
  ...
  var motions = {
    ...
    moveByPage : function(cm, head, motionArgs) {
      ...
      var curStart = head;
      var repeat = motionArgs.repeat;
      return cm.findPosV(curStart, (motionArgs.forward ? repeat : -repeat),
                         'page');
    },
    ...
    moveToMatchedSymbol : function(cm, head) {
      ...
      if (ch < lineText.length) {
        ...
        var re = /[<>]/.test(lineText[ch]) ? /[(){}[\]<>]/
                                           : /[(){}[\]]/; // ace_patch?
        var matched =
            cm.findMatchingBracket(Pos(line, ch + 1), {bracketRegex : re});
        return matched.to;
      } else {
        return cursor;
      }
    },
    moveToStartOfLine : function(_cm, head) { return Pos(head.line, 0); },
  ...
  function defineMotion(name, fn) { motions[name] = fn; }
  ...
  var operators = {
    change : function(cm, args, ranges) {
      ...
      vimGlobalState.registerController.pushText(
          args.registerName, 'change', text, args.linewise, ranges.length > 1);
    ...
    indent : function(cm, args, ranges) {
      ...
      var endLine = vim.visualBlock ? ranges[ranges.length - 1].anchor.line
                                    : ranges[0].head.line;
    ...
    yank : function(cm, args, ranges, oldAnchor) {
      ...
      var endPos = vim.visualMode ? cursorMin(vim.sel.anchor, vim.sel.head,
                                              ranges[0].head, ranges[0].anchor)
                                  : oldAnchor;
      vimGlobalState.registerController.pushText(
  ...
  function defineOperator(name, fn) { operators[name] = fn; }
  ...
  var actions = {
    ...
    enterInsertMode : function(cm, actionArgs, vim) {
      ...
      if (insertAt == 'eol') {
        ...
        if (!vim.visualBlock) {
          ...
          head = Pos(Math.min(sel.head.line, sel.anchor.line),
    ...
    toggleVisualMode : function(cm, actionArgs, vim) {
      ...
      if (!vim.visualMode) {
        ...
        vim.sel = {anchor : anchor, head : head};
    ...
    joinLines : function(cm, actionArgs, vim) {
      ...
      if (vim.visualMode) {
        ...
        curEnd =
            clipCursorToContent(cm, Pos(curStart.line + repeat - 1, Infinity));
      }
      var finalCh = 0;
    ...
    paste : function(cm, actionArgs, vim) {
      ...
      if (vim.visualMode) {
        ...
        if (blockwise) {
          ...
          selectionEnd =
              Pos(selectionStart.line + text.length - 1, selectionStart.ch);
    ...
    setRegister : function(
        _cm, actionArgs,
        vim) { vim.inputState.registerName = actionArgs.selectedCharacter; },
    ...
    incrementNumberToken : function(cm, actionArgs) {
      ...
      if (!actionArgs.backtrack && (end <= cur.ch))
        return;
      if (match) {
        var baseStr = match[2] || match[4] var digits =
                          match[3] || match[5] var increment =
                              actionArgs.increase ? 1 : -1;
        var base = {'0b' : 2,
                    '0' : 8,
                    '' : 10,
                    '0x' : 16}[baseStr.toLowerCase()];
        var number =
            parseInt(match[1] + digits, base) + (increment * actionArgs.repeat);
        numberStr = number.toString(base);
        var zeroPadding = baseStr ? new Array(digits.length - numberStr.length +
                                              1 + match[1].length)
                                        .join('0')
                                  : ''
  ...
  function defineAction(name, fn) { actions[name] = fn; }
  ...
  function commandMatches(keys, keyMap, context, inputState) {
    ...
    return {partial : partial.length && partial, full : full.length && full};
  }
  ...
  function copyCursor(cur) { return Pos(cur.line, cur.ch); }
  ...
  function cursorMin(cur1, cur2) {
    if (arguments.length > 2) {
      cur2 =
          cursorMin.apply(undefined, Array.prototype.slice.call(arguments, 1));
    }
  ...
  function cursorMax(cur1, cur2) {
    if (arguments.length > 2) {
      cur2 =
          cursorMax.apply(undefined, Array.prototype.slice.call(arguments, 1));
    }
  ...
  function getSelectedAreaRange(cm, vim) {
    ...
    var getCurrentSelectedAreaRange = function() {
      ...
      var selectionStart =
          cursorIsBefore(start.anchor, start.head) ? start.anchor : start.head;
      var selectionEnd =
          cursorIsBefore(end.anchor, end.head) ? end.head : end.anchor;
    ...
    var getLastSelectedAreaRange = function() {
      ...
      if (block) {
        ...
        selectionEnd =
            Pos(selectionStart.line + height, selectionStart.ch + width);
        var selections = [];
        ...
        if (lastSelection.visualLine) {
          ...
          selectionEnd =
              Pos(selectionEnd.line, lineLength(cm, selectionEnd.line));
        }
  ...
  function updateLastSelection(cm, vim) {
    ...
    if (vim.lastPastedText) {
      head =
          cm.posFromIndex(cm.indexFromPos(anchor) + vim.lastPastedText.length);
  ...
  function makeCmSelection(cm, sel, mode, exclusive) {
    ...
    if (mode == 'char') {
      ...
      return {ranges : [ {anchor : anchor, head : head} ], primary : 0};
    } else if (mode == 'line') {
      ...
      var ranges = [];
      for (var i = 0; i < height; i++) {
        ranges.push({anchor : Pos(top + i, left), head : Pos(top + i, right)});
      }
      return {ranges : ranges, primary : primary};
    }
  }
  function getHead(cm) {
  ...
  var findSymbolModes = {
    ...
    section : {
      ...
      isComplete : function(
          state) { return state.index === 0 && state.nextCh === state.symb; }
    },
    comment : {
      isComplete : function(state) {
    ...
    preprocess : {
      init : function(state) { state.index = 0; },
      isComplete : function(state) {
        if (state.nextCh === '#') {
  ...
  function moveToWord(cm, cur, repeat, forward, wordEnd, bigWord) {
    ...
    for (var i = 0; i < repeat; i++) {
      ...
      if (!word) {
        ...
        words.push(forward ? {line : cm.lastLine(), from : eodCh, to : eodCh}
                           : {line : 0, from : 0, to : 0});
        break;
      }
      words.push(word);
    ...
    if (forward && !wordEnd) {
      ...
      if (!shortCircuit &&
        ...
        // We did not start in the middle of a word. Discard the extra word at
        // the end.
        lastWord = words.pop();
      }
  ...
  function findSentence(cm, cur, repeat, dir) {
    ...
    function forward(cm, ln, pos, dir) {
      ...
      while (curr.line !== null) {
        ...
        } else if (isEndOfSentenceSymbol(curr.line[curr.pos]) && !stop &&
  ...
  SearchState.prototype = {
    getQuery : function() { return vimGlobalState.query; },
    setQuery : function(query) { vimGlobalState.query = query; },
    getOverlay : function() { return this.searchOverlay; },
    setOverlay : function(overlay) { this.searchOverlay = overlay; },
    isReversed : function() { return vimGlobalState.isReversed; },
    setReversed : function(reversed) { vimGlobalState.isReversed = reversed; },
    getScrollbarAnnotate : function() { return this.annotate; },
    setScrollbarAnnotate : function(annotate) { this.annotate = annotate; }
  };
  ...
  // Translates the replace part of a search and replace from ex (vim) syntax
  // into javascript form.  Similar to translateRegex, but additionally fixes
  // back references (translates '\[0..9]' to '$[0..9]') and follows different
  // rules for escaping '$'.
  var charUnescapes = {'\\n' : '\n', '\\r' : '\r', '\\t' : '\t'};
    ...
    return cm.operation(function() {
      ...
      for (var i = 0; i < repeat; i++) {
        ...
        if (!found) {
          ...
          cursor = cm.getSearchCursor(query, (prev) ? Pos(cm.lastLine())
                                                    : Pos(cm.firstLine(), 0));
          if (!cursor.find(prev)) {
            return;
          }
  ...
  var ExCommandDispatcher = function() { this.buildCommandMap_(); };
    ...
    map : function(lhs, rhs, ctx) {
      if (lhs != ':' && lhs.charAt(0) == ':') {
        ...
        if (rhs != ':' && rhs.charAt(0) == ':') {
          ...
          // Ex to key mapping
          this.commandMap_[commandName] =
              {name : commandName, type : 'exToKey', toKeys : rhs, user : true};
        }
      } else {
        if (rhs != ':' && rhs.charAt(0) == ':') {
          // Key to Ex mapping.
          ...
          // Key to key mapping
          var mapping = {keys : lhs, type : 'keyToKey', toKeys : rhs};
          if (ctx) {
            mapping.context = ctx;
          }
          defaultKeymap.unshift(mapping);
  ...
  var exCommands = {
    ...
    set : function(cm, params) {
      ...
      // Options passed through to the setOption/getOption calls. May be passed
      // in by the local/global versions of the set command
    ...
    substitute : function(cm, params) {
      ...
      var global = false;  // True to replace all instances on a line, false to
                           // replace only 1.
      if (tokens.length) {
        regexPart = tokens[0];
        if (getOption('pcre') && regexPart !== '') {
          regexPart =
              new RegExp(regexPart).source; // normalize not escaped characters
        }
    ...
    write : function(cm) {
      if (CodeMirror.commands.save) {
        ...
        // Saves to text area if no save command is defined and cm.save() is
        // available.
        cm.save();
      }
    },
    nohlsearch : function(cm) { clearSearchHighlight(cm); },
    yank : function(cm) {
      var cur = copyCursor(cm.getCursor());
      var line = cur.line;
      var lineText = cm.getLine(line);
      vimGlobalState.registerController.pushText('0', 'yank', lineText, true,
                                                 true);
    },
   ...
   */
  function doReplace(cm, confirm, global, lineStart, lineEnd, searchCursor,
                     query, replaceWith, callback) {
  ...
  CodeMirror.keyMap
      .vim = {attach : attachVimMap, detach : detachVimMap, call : cmKey};
  ...
  function _mapCommand(command) { defaultKeymap.unshift(command); }
  ...
  function updateFakeCursor(cm) {
    ...
    vim.fakeCursor =
        cm.markText(from, to, {className : 'cm-animate-fat-cursor'});
  }
  function handleExternalSelection(cm, vim, keepHPos) {
    ...
    if (vim.visualMode) {
      ...
      // Reset lastHPos if selection was modified by something outside of vim
      // mode e.g. by mouse.
  ...
  Vim.handleKey = function(cm, key, origin) {
    return cm.operation(function() { return handleKey(cm, key, origin); },
                        true);
    ...
    // ace commands like ctrl-alt-l may bring visualMode and selection out of
    // sync
  ...
  exports.handler = {
    ...
    handleKeyboard : function(data, hashId, key, keyCode, e) {
      ...
      // in non-insert mode we try to find the ascii key corresponding to the
      // text in textarea this is needed because in languages that use latin
      // alphabet we want to get the key that browser sends to the textarea and
      // in non
      if (!vim.insertMode) {
        if (hashId == -1) {
          ...
          if (data.inputKey == key && data.inputHash == hashId &&
              data.inputChar) {
      ...
      if (key == "c" && hashId == 1) { // key == "ctrl-c"
        if (!useragent.isMac && editor.getCopyText()) {
          editor.once("copy",
                      function() { editor.selection.clearSelection(); });
  ...
  Vim.defineEx('write', 'w',
               function() { console.log(':write is not implemented') });
  defaultKeymap.push({
    keys : 'zc',
    type : 'action',
    action : 'fold',
  ...
  actions.fold = function(cm, actionArgs, vim) {
    cm.ace.execCommand(
        [ 'toggleFoldWidget', 'toggleFoldWidget', 'foldOther',
          'unfoldall' ][(actionArgs.all ? 2 : 0) + (actionArgs.open ? 1 : 0)]);
  };
