...
define(function(require, exports, module) {
  ...
  CodeMirror.defineMode = function() {} for (var key in vim.CodeMirror)
  ...
  vim.CodeMirror.Vim.defineEx('write', 'w',
                              function(cm) { CodeMirror.commands.save(cm); });

  // cm.setBookmark({ch: 5, line: 0})
  // cm.setBookmark({ch: 4, line: 0})
  // cm.replaceRange("x-", {ch: 4, line: 0}, {ch: 5, line: 0});
  // [editor.$vimModeHandler.cm.marks[0].find(),editor.$vimModeHandler.cm.marks[1].find()]
  ...
  var endOfDocument =
      makeCursor(lines.length - 1, lines[lines.length - 1].length);
  ...
  var oChars = [
    new Pos(charLine.line, 1), new Pos(charLine.line, 3),
    new Pos(charLine.line, 7)
  ];
  var pChars = [
    new Pos(charLine.line, 2), new Pos(charLine.line, 4),
    new Pos(charLine.line, 6), new Pos(charLine.line, 8)
  ];
  var numChars = [
    new Pos(charLine.line, 10), new Pos(charLine.line, 12),
    new Pos(charLine.line, 14), new Pos(charLine.line, 16),
  ...
  function copyCursor(cur) { return new Pos(cur.line, cur.ch); }
  ...
  function testVim(name, run, opts, expectedFail) {
    ...
    return test('vim_' + name, function() {
      ...
      function doInsertModeKeysFn(cm) {
        return function(args) {
          ...
          for (var i = 0; i < arguments.length; i++) {
            ...
            var handled = CodeMirror.lookupKey(key, cm.getOption('keyMap'),
                                               executeHandler, cm);
            // Record for insert mode.
            if (handled == "handled" && cm.state.vim.insertMode &&
                arguments[i] != 'Esc') {
              var lastChange = CodeMirror.Vim.getVimGlobalState_()
                                   .macroModeState.lastInsertModeChanges;
              if (lastChange && (key.indexOf('Delete') != -1 ||
                                 key.indexOf('Backspace') != -1)) {
      ...
      function fakeOpenDialog(result) {
        return function(text, callback) { return callback(result); }
      }
      function fakeOpenNotification(matcher) {
        return function(text) { matcher(text); }
      }
      var helpers = {
        doKeys : doKeysFn(cm),
        ...
        getRegisterController :
            function() { return CodeMirror.Vim.getRegisterController(); }
  ...
  testJumplist('jumplist_{', [ '{', '<C-o>' ], [ 1, 5 ], [ 1, 5 ]);
  testJumplist('jumplist_}', [ '}', '<C-o>' ], [ 1, 5 ], [ 1, 5 ]);
  testJumplist('jumplist_\'', [ 'm', 'a', 'h', '\'', 'a', 'h', '<C-i>' ],
               [ 1, 0 ], [ 1, 5 ]);
  testJumplist('jumplist_`', [ 'm', 'a', 'h', '`', 'a', 'h', '<C-i>' ],
               [ 1, 5 ], [ 1, 5 ]);
  ...
  testJumplist('jumplist_N', [ '#', 'N', '<C-o>' ], [ 1, 1 ], [ 2, 3 ]);
  testJumplist('jumplist_repeat_<c-o>', [ '*', '*', '*', '3', '<C-o>' ],
               [ 2, 3 ], [ 2, 3 ]);
  testJumplist('jumplist_repeat_<c-i>',
               [ '*', '*', '*', '3', '<C-o>', '2', '<C-i>' ], [ 5, 0 ],
               [ 2, 3 ]);
  testJumplist('jumplist_repeated_motion', [ '3', '*', '<C-o>' ], [ 2, 3 ],
               [ 2, 3 ]);
  ...
  function makeCursor(line, ch) { return new Pos(line, ch); }
  ...
  testMotion('k_repeat_clip', [ '1000', 'k' ], makeCursor(0, 4),
             makeCursor(2, 4));
  testMotion('w', 'w', word1.start);
  testMotion('keepHPos', [ '5', 'j', 'j', '7', 'k' ], makeCursor(8, 12),
             makeCursor(12, 12));
  testMotion('keepHPosEol', [ '$', '2', 'j' ], makeCursor(2, 18));
  testMotion('w_multiple_newlines_no_space', 'w', makeCursor(12, 2),
             makeCursor(11, 2));
  testMotion('w_multiple_newlines_with_space', 'w', makeCursor(14, 0),
             makeCursor(12, 51));
  ...
  testMotion('w_start_to_end', [ '1000', 'w' ], endOfDocument,
             makeCursor(0, 0));
  ...
  testMotion('e_start_to_end', [ '1000', 'e' ], endOfDocument,
             makeCursor(0, 0));
  ...
  testMotion('b_end_to_start', [ '1000', 'b' ], makeCursor(0, 0),
             endOfDocument);
  ...
  testMotion('ge_startOfDocument', [ 'g', 'e' ], makeCursor(0, 0),
             makeCursor(0, 0));
  testMotion('ge_end_to_start', [ '1000', 'g', 'e' ], makeCursor(0, 0),
             endOfDocument);
  ...
  testMotion('G', 'G',
             makeCursor(lines[lines.length - 1].line,
                        lines[lines.length - 1].textStart),
             makeCursor(3, 1));
  testMotion('G_repeat', [ '3', 'G' ],
             makeCursor(lines[2].line, lines[2].textStart));
  ...
  testMotion('_', [ '6', '_' ], makeCursor(5, lines[5].textStart),
             makeCursor(0, 8));
  ...
  testMotion('T_repeat', [ '2', 'T', 'p' ], offsetCursor(pChars[0], 0, 1),
             pChars[2]);
  ...
  // Make sure that moving down after going to the end of a line always leaves
  // you at the end of a line, but preserves the offset in other cases
  testVim('Changing lines after Eol operation', function(cm, vim, helpers) {
    ...
    helpers.doKeys([ 'h' ]);
    helpers.doKeys([ 'j' ]);
    // After moving back one space and then down, since line 4 is shorter than
    // line 2, we should be at Eol of line 2 - 1
    helpers.assertCursorAt(new Pos(3, lines[3].length - 1));
    helpers.doKeys([ 'j' ]);
    helpers.doKeys([ 'j' ]);
    // After moving down again, since line 3 has enough characters, we should be
    // back to the same place we were at on line 1
  ...
  testVim('j_k_and_gj_gk', function(cm, vim, helpers) {
    ...
    // should move to character 177 on line 2 (j/k preserve character index
    // within line)
    helpers.doKeys('j');
    // due to different line wrapping, the cursor can be on a different screen-x
    // now gj and gk preserve screen-x on movement, much like moveV
  ...
  testVim('gj_gk', function(cm, vim, helpers) {
    ...
    is(topLeftCharCoords.left == endingCharCoords.left,
  ...
  testVim('dw_eol', function(cm, vim, helpers) {
    // Assert that dw does not delete the newline if last word to delete is at
    // end of line.
    cm.setCursor(0, 1);
    helpers.doKeys('d', 'w');
    eq(' \nword2', cm.getValue());
  ...
  testVim('dw_eol_with_multiple_newlines', function(cm, vim, helpers) {
    // Assert that dw does not delete the newline if last word to delete is at
    // end of line and it is followed by multiple newlines.
    cm.setCursor(0, 1);
    helpers.doKeys('d', 'w');
    eq(' \n\nword2', cm.getValue());
    var register = helpers.getRegisterController().getRegister();
  ...
  // Yank commands should behave the exact same as d commands, expect that
  ...
  testEdit('dab_on_(_should_delete_around_()block', 'o( in(abc) )', /\(a/,
           'dab', 'o( in )');

  //  delete around and inner B.
  testEdit('daB_on_{_should_delete_around_{}block', 'o{ in{abc} }', /{a/, 'daB',
           'o{ in }');
  testEdit('diB_on_{_should_delete_inner_{}block', 'o{ in{abc} }', /{a/, 'diB',
           'o{ in{} }');

  testEdit('da{_on_{_should_delete_inner_block', 'o{ in{abc} }', /{a/, 'da{',
           'o{ in }');
  testEdit('di[_on_(_should_not_delete', 'foo (bAr) baz', /\(/, 'di[',
           'foo (bAr) baz');
  testEdit('di[_on_)_should_not_delete', 'foo (bAr) baz', /\)/, 'di[',
           'foo (bAr) baz');
  testEdit('da[_on_(_should_not_delete', 'foo (bAr) baz', /\(/, 'da[',
           'foo (bAr) baz');
  testEdit('da[_on_)_should_not_delete', 'foo (bAr) baz', /\)/, 'da[',
           'foo (bAr) baz');
  testMotion('di(_outside_should_stay', [ 'd', 'i', '(' ], new Pos(0, 0),
             new Pos(0, 0));
  ...
  testVim(']p_with_tab_indents', function(cm, vim, helpers) {
    helpers.getRegisterController().pushText('"', 'yank',
                                             '\t\tabc\n\t\t\tdef\n', true);
    helpers.doKeys(']', 'p');
    eq('\t___\n\tabc\n\t\tdef', cm.getValue());
  ...
  testVim('jumpToMark_next_line_action', function(cm, vim, helpers) {
    ...
    var expected = ' (a) [b] {c} '
    eq(actual, expected,
       "Deleting while jumping to the next mark line failed.");
  });
  ...
  testVim('v_paste_from_register', function(cm, vim, helpers) {
    ...
    cm.openNotification = helpers.fakeOpenNotification(function(
        text) { is(/a\s+register/.test(text)); });
  ...
  testVim(':_register', function(cm, vim, helpers) {
    ...
    cm.openDialog = helpers.fakeOpenDialog('registers');
    cm.openNotification = helpers.fakeOpenNotification(function(
        text) { is(/:\s+bar/.test(text)); });
    helpers.doKeys(':');
  }, {value : ''});
  testVim('search_register_escape', function(cm, vim, helpers) {
    // Check that the register is restored if the user escapes rather than
    // confirms.
    cm.openDialog = helpers.fakeOpenDialog('waldo');
    helpers.doKeys('/');
    var onKeyDown;
    var onKeyUp;
    var KEYCODES = {f : 70, o : 79, Esc : 27};
  ...
  testVim('exCommand_history', function(cm, vim, helpers) {
    ...
    var onKeyUp;
    var input = '';
    var keyCodes = {Up : 38, Down : 40, s : 115};
    cm.openDialog = function(template, callback, options) {
      onKeyUp = options.onKeyUp;
      onKeyDown = options.onKeyDown;
    };
  ...
  testVim('search_clear', function(cm, vim, helpers) {
    var onKeyDown;
    var input = '';
    var keyCodes = {Ctrl : 17, u : 85};
    cm.openDialog = function(template, callback,
  ...
  testVim('._insert_cw_repeat', function(cm, vim, helpers) {
    // For some reason, repeat cw in desktop VIM will does not repeat insert
    // mode changes. Will conform to that behavior.
    helpers.doKeys('c', 'w');
    helpers.doKeys('test');
    helpers.doKeys('<Esc>');
    cm.setCursor(0, 4);
  ...
  forEach([ 'zb', 'zz', 'zt', 'z-', 'z.', 'z<CR>' ], function(e, idx) {
    ...
    }, {value : (function() { return new Array(500).join('\n'); })()});
  });
  ...
  testVim('zb<zz',
          function(cm, vim, helpers) { eq(zVals[0] < zVals[1], true); });
  testVim('zz<zt',
          function(cm, vim, helpers) { eq(zVals[1] < zVals[2], true); });
  testVim('zb==z-', function(cm, vim, helpers) { eq(zVals[0], zVals[3]); });
  testVim('zz==z.', function(cm, vim, helpers) { eq(zVals[1], zVals[4]); });
  testVim('zt==z<CR>', function(cm, vim, helpers) { eq(zVals[2], zVals[5]); });
  ...
  testVim('moveTillCharacter', function(cm, vim, helpers) {
    ...
    helpers.doKeys('z');
    eq('The quick box \n', cm.getValue());
    // After all that, the search for 'q' should still be active, so the 'N'
    // command can run it again in reverse. Use that to delete everything back
    // to the 'q'.
    helpers.doKeys('d');
    helpers.doKeys('N');
    eq('The ox \n', cm.getValue());
  ...
  testVim('ex_global_confirm', function(cm, vim, helpers) {
    ...
    var KEYCODES = {a : 65, n : 78, q : 81, y : 89};
  ...
  isAce || testVim("ex_escaped_ampersand_should_not_substitute_pcre",
  ...
  function testSubstituteConfirm(name, command, initialValue, expectedValue,
                                 keys, finalPos) {
    testVim(name, function(cm, vim, helpers) {
      var savedOpenDialog = cm.openDialog;
      ...
      cm.openDialog = function(template, callback,
                               options) { recordedCallback = callback; }
                      // Do first openDialog.
                      helpers.doKeys(':');
      ...
      CodeMirror.keyName = function(e) { return e.key; } keys =
          keys.toUpperCase();
  ...
  testSubstituteConfirm('ex_substitute_confirm_emptydoc', '%s/x/b/c', '', '',
                        '', makeCursor(0, 0));
  testSubstituteConfirm('ex_substitute_confirm_nomatch', '%s/x/b/c',
                        'ba a\nbab', 'ba a\nbab', '', makeCursor(0, 0));
  testSubstituteConfirm('ex_substitute_confirm_accept', '%s/a/b/cg',
                        'ba a\nbab', 'bb b\nbbb', 'yyy', makeCursor(1, 1));
  testSubstituteConfirm('ex_substitute_confirm_random_keys', '%s/a/b/cg',
                        'ba a\nbab', 'bb b\nbbb', 'ysdkywerty',
                        makeCursor(1, 1));
  ...
  testSubstituteConfirm('ex_substitute_confirm_accept_then_all', '%s/a/b/cg',
  ...
  testSubstituteConfirm('ex_substitute_confirm_oneline', '1s/a/b/cg',
                        'ba a\nbab', 'bb b\nbab', 'yl', makeCursor(0, 3));
  testSubstituteConfirm('ex_substitute_confirm_range_accept', '1,2s/a/b/cg',
                        'aa\na \na\na', 'bb\nb \na\na', 'yyy',
                        makeCursor(1, 0));
  testSubstituteConfirm('ex_substitute_confirm_range_some', '1,3s/a/b/cg',
                        'aa\na \na\na', 'ba\nb \nb\na', 'ynyy',
                        makeCursor(2, 0));
  testSubstituteConfirm('ex_substitute_confirm_range_all', '1,3s/a/b/cg',
                        'aa\na \na\na', 'bb\nb \nb\na', 'a', makeCursor(2, 0));
  testSubstituteConfirm('ex_substitute_confirm_range_last', '1,3s/a/b/cg',
                        'aa\na \na\na', 'bb\nb \na\na', 'yyl',
                        makeCursor(1, 0));
  //:noh should clear highlighting of search-results but allow to resume search
  //through n
  testVim('ex_noh_clearSearchHighlight', function(cm, vim, helpers) {
    cm.openDialog = helpers.fakeOpenDialog('match');
    helpers.doKeys('?');
    helpers.doEx('noh');
    eq(vim.searchState_.getOverlay(), null,
  ...
  testVim('ex_set_string', function(cm, vim, helpers) {
    ...
    eq('c',
       CodeMirror.Vim.getOption('testopt', cm, {scope : 'local'})); // local
    eq('c',
       CodeMirror.Vim.getOption('testopt', cm, {scope : 'global'})); // global
  ...
  testVim('ex_set_filetype', function(cm, vim, helpers) {
    CodeMirror.defineMode('test_mode', function() {
      return {token : function(stream) { stream.match(/^\s+|^\S+/); }};
    });
  ...
  testVim(
      'ex_unmap_key2key_does_not_remove_default', function(cm, vim, helpers) {
        helpers.doEx('unmap a');
        helpers.doKeys('a');
        eq('vim-insert', cm.getOption('keyMap'));
        CodeMirror.Vim.mapclear();
  ...
  testVim('ex_map_key2key_to_colon', function(cm, vim, helpers) {
    ...
    cm.openDialog = function() { dialogOpened = true; } helpers.doKeys(';');
  ...
  // For now, this test needs to be last because it messes up : for future
  // tests.
