...
function checkModes() {
  ...
  modeList().forEach(function(modeName, i) {
    console.log(padNumber(i + 1, 3) + ") check: \u001b[33m" + modeName +
                "\u001b[0m");
    try {
  ...
  function testComments(desc, fn, tokenizer, modeName) {
    if (desc) {
      if (Array.isArray(desc)) {
        desc.forEach(function(x) { fn(tokenizer, x, modeName); });
      } else {
  ...
  function testBlockComment(tokenizer, blockComment, modeName) {
    ...
    var isBroken =
        data.tokens.some(function(t) { return !/comment/.test(t.type); });
    if (isBroken) {
  ...
  function testBrackets(mode, modeName) {
    ...
    var isInvalid = session.getTokens(0).some(function(
        t) { return /invalid|illegal|string/.test(t.type); });
    if (isInvalid)
      return;
...
function generateTestData(names, force) {
  ...
  docs.forEach(function(docName) {
    ...
    if (oldOutput &&
        JSON.stringify(JSON.parse(jsonStr)) == JSON.stringify(oldOutput))
      return;
...
function test(startAt) {
  var modes =
      fs.readdirSync(cwd)
          .map(function(x) { return (x.match(/tokens_(.*).json/) || {})[1]; })
...
function testMode(modeName, i) {
  console.log(padNumber(i + 1, 3) + ") testing: \u001b[33m" + modeName +
              "\u001b[0m");
  ...
  data.forEach(function(lineData) {
    ...
    var err = testEqual([
      ...
      lineData.types, types, lineData.values, values
    ]);
...
function padNumber(num, digits) { return ("      " + num).slice(-digits); }
...
function maybeCatastrophicBacktracking(regex) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (token.type == "quantifier" && quantifiedGroups.length >= 1 &&
        token.value != "?") {
      return groupIndex;
    }
  }
