define(function(require, exports, module) {
  ...
  (function(root, name,
            factory) { factory(exports) }(this, 'luaparse', function(exports) {
    'use strict';
    ...
    var ast = exports.ast = {
      labelStatement : function(
          label) { return {type : 'LabelStatement', label : label}; }
      ...
      gotoStatement : function(
          label) { return {type : 'GotoStatement', label : label}; }

      ,
      returnStatement : function(
          args) { return {type : 'ReturnStatement', 'arguments' : args}; }

      ,
      ifStatement : function(
          clauses) { return {type : 'IfStatement', clauses : clauses}; },
      ...
      elseClause : function(body) { return {type : 'ElseClause', body : body}; }
      ...
      doStatement : function(
          body) { return {type : 'DoStatement', body : body}; }
      ...
      localStatement : function(variables, init) {
        return {type : 'LocalStatement', variables : variables, init : init};
      }
      ...
      ,
      chunk : function(body) { return {type : 'Chunk', body : body}; }

      ,
      identifier : function(name) { return {type : 'Identifier', name : name}; }
      ...
      ,
      tableKey : function(
          key, value) { return {type : 'TableKey', key : key, value : value}; },
      tableKeyString : function(key, value) {
        return {type : 'TableKeyString', key : key, value : value};
      },
      tableValue : function(
          value) { return {type : 'TableValue', value : value}; }
      ...
      indexExpression : function(base, index) {
        return {type : 'IndexExpression', base : base, index : index};
      }
      ...
      tableCallExpression : function(base, args) {
        return {type : 'TableCallExpression', base : base, 'arguments' : args};
      }
      ...
      ,
      comment : function(
          value, raw) { return {type : 'Comment', value : value, raw : raw}; }
    };
    ...
    function sprintf(format) {
      ...
      format = format.replace(
          /%(\d)/g,
          function(match, index) { return '' + args[index - 1] || ''; });
      return format;
    }
    ...
    // #### Raise an exception.
    //
    // Raise an exception by passing a token, a string format and its parameters.
    //
    ...
    // in an array which later will be appended to the chunk object. If
    // disabled, they will simply be disregarded.
    //
    // When the lexer has derived a valid token, it will be returned as an
    // object containing its value and as well as its position in the input
    // string (this is always enabled to provide proper debug messages).
    //
    // `lex()` starts lexing and returns the following token in the stream.

    var index, token, previousToken, lookahead, comments, tokenStart, line,
        lineStart;
    ...
    // tracking the encountered newlines. Newlines are also tracked in all
    ...
    function scanStringLiteral() {
      var delimiter = input.charCodeAt(index++), stringStart = index,
          string = '', charCode;
    ...
    // all three parts separately and then sum them up at the end of the
    ...
    function scanComment() {
      ...
      var character = input.charAt(index), content = '', isLong = false,
          commentStart = index, lineStartComment = lineStart,
          lineComment = line;
      ...
      if (options.comments) {
        ...
        // `Marker`s depend on tokens available in the parser and as comments
        // are intercepted in the lexer all location data is set manually.
        if (options.locations) {
          node.loc = {
            start :
                {line : lineComment, column : tokenStart - lineStartComment},
    ...
    function isWhiteSpace(charCode) {
      return 9 === charCode || 32 === charCode || 0xB === charCode ||
             0xC === charCode;
    }
    ...
    function isKeyword(id) {
      ...
      case 5:
        return 'break' === id || 'local' === id || 'until' === id ||
               'while' === id;
      case 6:
    ...
    // -----------------
    //
    // Locations are stored in FILO-array as a `Marker` object consisting of
    // both `loc` and `range` data. Once a `Marker` is popped off the list an
    // end location is added and the data is attached to a syntax node.

    var locations = [], trackLocations;

    function createLocationMarker() { return new Marker(token); }
    ...
    function parseAssignmentOrCallStatement() {
      // Keep a reference to the previous token for better error messages in
      // case of invalid statement
    ...
    function parseFunctionDeclaration(name, isLocal) {
      ...
      if (!consume(')')) {
        // Arguments are a comma separated list of identifiers, optionally
        // ending with a vararg.
        while (true) {
          if (Identifier === token.type) {
    ...
    function parseSubExpression(minPrecedence) {
      ...
      while (true) {
        ...
        if (trackLocations)
          locations.push(marker);
        expression =
            finishNode(ast.binaryExpression(operator, expression, right));
      }
      return expression;
    }
    ...
    function parsePrimaryExpression() {
      var literals = StringLiteral | NumericLiteral | BooleanLiteral |
                     NilLiteral | VarargLiteral,
          value = token.value, type = token.type, marker;
