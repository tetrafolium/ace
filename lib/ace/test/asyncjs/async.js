...
define(function(require, exports, module) {
  var STOP = exports.STOP = {}
    ...
    if (typeof source == "function")
      this.source = {next : source} else this.source = source
  }

  ;
  (function() {
    this.next = function(callback) { this.source.next(callback) }
      ...
      if (!mapper)
        ...
        var source = this.source this.next = function(callback) {
        ...
        source.next(function handler(err, value) {
          if (err)
            callback(err)
              ...
              fn(value, function(err) { callback(err, value) })
            }
        })
      } return new this.constructor(this)
    }
      ...
      var done = false this.next = function(callback) {
        if (done)
          return callback(STOP)
      ...
      return this.map(function(fn, next) {
        ...
        fn.call(context, function(err, value) { next(err, value) })
      })
    }
      ...
      var source = generators[index++]
                   ...
                   return new this.constructor(function(callback) {
                     source.next(function handler(err, value) {
                       if (err) {
                         if (err == STOP) {
                           source = generators[index++] if (!source)
                           return callback(STOP)
      ...
      return new this.constructor(function(callback) {
        exports.list(generators)
            .map(function(gen, next) { gen.next(next) })
            .toArray(callback)
      })
    }
          ...
          spliced.next(function(err, value) {
            if (err == STOP) {
              ...
              callback(err, value)
                                                ...
                                                this.sort =
                                                    function(compare) {
      var self = this var arrGen this.next = function(callback) {
        if (arrGen)
            ...
            if (err)
              callback(err)
                ...
                arrGen.next(callback)
                                                    ...
                                                    this.join =
                                                        function(separator) {
      return this.$arrayOp(Array.prototype.join,
      ...
      return this.$arrayOp(Array.prototype.reverse)
                                                            ...
                                                            this.$arrayOp =
                                                                function(
                                                                    arrayMethod,
                                                                    args) {
      var self = this var i = 0 this.next = function(callback) {
        if (i++ > 0)
          return callback(STOP)
      ...
      var values = [] var errors = [] var source = this.source
  ...
  var makeAsync = exports.makeAsync =
      function(args, fn, context) {
    if (fn.length > args)
      return fn
      else {
        return function() {
          var value
          var next = arguments[args] try {
    ...
    return exports.list(keys, construct)
                   ...
                   *
                   * Return a generator containing an arithmetic progression of
                   * integers. range(i, j) returns [i, i+1, i+2, ..., j-1] start
                   * (!) defaults to 0. When step is given, it specifies the
                   * increment (or decrement).
                   */
                  exports.range =
                      function(start, stop, step, construct) {
    var construct = construct || exports.Generator
    start = start || 0
    step = step || 1
    ...
    if (arguments.length > 1)
      ...
      else return first.map(function(item, next) { next(null, [ item ]) })
  }
