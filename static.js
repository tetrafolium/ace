...
var http = require("http"), path = require("path"), url = require("url"),
    fs = require("fs"), port = process.env.PORT || 8888,
      ...
      if (req.method == "PUT") {
        if (!allowSave)
          return error(res, 404,
                       "Saving not allowed pass --allow-save to enable");
  ...
  var html =
      files
          .map(function(name) {
            ...
            var index =
                name == "." ? 2 : name == ".." ? 3 : stat.isDirectory() ? 1 : 0;
...
function save(req, res, filePath) {
  ...
  req.on("error", function() { error(res, 404, "Could't save file"); });
  req.on("end", function() {
    try {
