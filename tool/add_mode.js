...
function main(displayName, extRe) {
  ...
  var demoFileName =
      demoFileExt[0] == "^" ? demoFileExt.substr(1) : name + "." + demoFileExt;
  ...
  text = lib.fillTemplate(template, {languagename : name, snippets : ""});
  ...
  modelist = modelist.replace(
      /(supportedModes = {\n)([\s\S]*?)(\n^};)/m, function(_, m1, m2, m3) {
        ...
        var padding =
            Array(Math.max(offset - displayName.length - 1, 0) + 1).join(" ");
        var newLang =
            "    " + displayName + ":" + padding + "[\"" + extRe + "\"]";
        langs = langs.concat(newLang).map(function(x) {
          return {value : x, id : x.match(/[^"':\s]+/)[0].toLowerCase()};
        });
        ...
        langs = langs.sort(function(a, b) { return a.id.localeCompare(b.id); })
