define(function(require, exports, module) {
  /**
   * creates globals intentionally to make things easily accessible from
   * console *
   */
  ...
  editor1.saveButton.onclick = function() {
    doclist.saveDoc({
      ...
      session : editor1.session
                    ...
                    function(err) { handleSaveResult(err, editor1); });
  };
  editor1.commands.bindKey({win : "Ctrl-S", mac : "Cmd-s"},
                           editor1.saveButton.onclick);
  editor2.saveButton.onclick = function() {
    doclist.saveDoc(docEl.value,
                    function(err) { handleSaveResult(err, editor2); });
  };
  editor2.commands.bindKey({win : "Ctrl-S", mac : "Cmd-s"},
  ...
  themelist.themes.forEach(function(x) {
    themes[x.isDark ? "Dark" : "Bright"].push(
        {caption : x.caption, value : x.name});
  });
  ...
  function run() {
    ...
    src = src.replace("define(", 'define("' + path +
