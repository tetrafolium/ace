...
var Tokenizer =
    require("../lib/ace/tokenizer_dev")
        .Tokenizer; // todo can't use tokenizer because of max token count
var TextHighlightRules =
    require("../lib/ace/mode/text_highlight_rules").TextHighlightRules;
...
r.$rules = {
  start : [
    ...
    {include : "groups", merge : false}, {include : "xGroup", merge : true}
  ],
  charTypes : [
    {
      token : "char",
      regex : /\\([tvnrbfae]|[0-8]{1,3}|x[\dA-Fa-f]{2}|x7[\dA-Fa-f]{7})/,
      merge : false
    }, // todo \cx
  ...
  groups : [ {
    ...
    onMatch : function(val, state, stack) {
      ...
      var t = {
        depth : 0,
        type : isStart ? "group.start" : "group.end",
        value : val
      };
      t.groupType = val[2];
  ...
  xGroup : [
    {
      ...
      onMatch : function(val, state,
                         stack) { return stack.xGroup ? [] : "text" },
      merge : true
    },
    {
      token : "text",
...
function tokenize(str) { return tmReTokenizer.getLineTokens(str).tokens; }

function toStr(tokens) {
  return tokens.map(function(x) { return x.value }).join("")
}
