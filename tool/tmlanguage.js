...
function simplifyNonCapturingGroups(str) {
  var tokens = tokenize(str);
  var t = tokens[0] || {};
  if (t.type == "group.start" && t.value == "(?:" && t.end == last(tokens)) {
    t.value = t.end.value = "";
  }
  var i = 0;
  function iter(f) {
...
function fixGroups(captures, defaultName, regex) {
  ...
  iter(function(t) {
    if (t.type == "group.end" && t.start.value == "(" &&
        peek().type == "quantifier") {
      peek().value += ")";
      t.start.value += "(?:";
    }
  });
  ...
  return {names : names, regex : toStr(tokens)};
}

/***** converter */

function logDebug(string, obj) { console.log(string, obj); }
...
function processPatterns(pl) { return pl.map(processPattern); }
...
function simpleRule(regex, name, captures) {
  ...
  if (captures) {
    var tokenArray = [];
    Object.keys(captures).forEach(function(
        x) { tokenArray[x] = captures[x] && captures[x].name; });
...
//
function extractPatterns(tmRules) { return processRules(tmRules); }
...
function detectLoops(states) {
  ...
  function addPath(start, path) {
    ...
    if (i > -1 && i != path.length - 1 || start == "$self" ||
        start == "$base") {
      if (i != -1)
        path = path.slice(i);
    ...
    if (!node || !node.refs || !node.refs.length || path.length > 30)
      return;
    node.refs.forEach(function(x) { addPath(x, path.concat()); });
  }
  addPath("start", []);
...
var modeHighlightTemplate =
    fs.readFileSync(__dirname + "/templates/highlight_rules.js", "utf8");
...
function fetchAndConvert(name) {
  ...
  if (/^http/.test(name)) {
    if (/:\/\/github.com/.test(name)) {
      name =
          name.replace(/\/blob\//, "/").replace("github.com", "raw.github.com");
    }
    return lib.download(name,
                        function(data) { convertTmLanguage(name, data); });
  }
...
function convertTmLanguage(name, langStr) {
  parseLanguage(langStr, function(language) {
    var highlighterFilename =
        lib.snakeCase(language.name).replace(/[^\w]/g, "");
    var languageNameSanitized =
        lib.camelCase(language.name).replace(/[^\w]/g, "");

    require("./add_mode")(languageNameSanitized,
                          (language.fileTypes || []).join("|"));

    var highlighterFile = pathlib.normalize(
        lib.AceLib + "ace/mode/" + highlighterFilename + "_highlight_rules.js");
    var modeFile = pathlib.normalize(lib.AceLib + "ace/mode/" +
                                     highlighterFilename + ".js");
...
if (!module.parent) {
  ...
  if (args.length < 1) {
    console.error(
        "Usage: node tmlanguage.js [--dev] path/or/url/to/syntax.file ...");
    process.exit(1);
  }
