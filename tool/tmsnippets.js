...
function readSnippet(path, name) {
  ...
  plist.parseString(plistString, function(_, plist) { snippets.push(plist) })
}
...
function readSnippetsInDir(path) {
  if (fs.statSync(path).isDirectory()) {
    path += "/"
    snippets.push(path)
    fs.readdirSync(path).forEach(function(name) { readSnippet(path, name) })
  } else {
    readSnippet(path)
  }
}
...
snippets = snippets.map(function(s) {
  if (s.length == 1)
    s = s[0] if (s.scope)
    s.scope = s.scope.replace(/source\./g, "") delete s.uuid
    return s
})
