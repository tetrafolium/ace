...
function parseTheme(themeXml, callback) {
  parseString(themeXml, function(_, theme) { callback(theme[0]) });
}
...
function extractStyles(theme) {
  ...
  var colors = {
    ...
    "active_line" :
        parseColor(globalSettings.lineHighlight) || defaultGlobals.active_line,
...
function fillTemplate(template, replacements) {
  return template.replace(/%(.+?)%/g,
                          function(str, m) { return replacements[m] || ""; });
}
...
function normalizeStylesheet(rules) {
  ...
  for (var i = rules.length; i--;) {
    ...
    rules[i].selectors =
        s.filter(function(x, i) { return x && x != s[i + 1]; });
  }
  return rules;
}
...
function convertBuiltinTheme(name) {
  return convertTheme(name,
                      __dirname + "/tmthemes/" + themes[name] + ".tmTheme",
                      __dirname + "/../lib/ace/theme");
}
...
function convertTheme(name, tmThemePath, outputDirectory) {
  ...
  parseTheme(tmTheme, function(theme) {
    ...
    for (var i in supportedScopes) {
      if (!styles[i])
        continue;
      css += "." + styles.cssClass + " " + i.replace(/^|\./g, ".ace_") + "{" +
             styles[i] + "}";
    }
    ...
    try {
      ...
      for (var i = 0; i < newRules.length; i++) {
        ...
        for (var j = 0; j < oldRules.length; j++) {
          ...
          newSelectors = newSelectors.filter(function(
              s) { return oldSelectors.indexOf(s) == -1; })
    ...
    var js = fillTemplate(jsTemplate, {
      name : name,
      css : 'require("../requirejs/text!./' + name +
                '.css")', // quoteString(css), //
...
if (process.argv.length > 2) {
  ...
  if (args.length < 3) {
    console.error(
        "Usage: node tmtheme.js [theme_name, path/to/theme.tmTheme path/to/output/directory]");
    process.exit(1);
  }
  var name = args[0];
...
if (Object.keys(unsupportedScopes).length > 0) {
  ...
  console.log(
      "It's safe to ignore these, but they may affect your syntax highlighting if your mode depends on any of these rules.");
  console.log(
      "Refer to the docs on ace.ajax.org for information on how to add a scope to the CSS generator.");
}
